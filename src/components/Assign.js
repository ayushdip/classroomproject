...
const Assign = () => {
    ...
    function addComment(){
        if(inp){
            db.collection('classrom').doc(id).collection('posts').doc(assignId).collection('comments').add({
    ...
    useEffect(()=>{
        db.collection('classrom').doc(id).collection('posts').doc(assignId)
        .onSnapshot((snapshot)=>setMat({id : snapshot.id,data : snapshot.data()}))
        db.collection('classrom').doc(id).collection('posts').doc(assignId).collection('comments')
